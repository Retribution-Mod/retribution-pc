...
import { isRetributionPluginDev, isPluginDev } from "@utils/misc";
...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "RetributionDonor":
            return (
                <span style={{ order: settings.store.RetributionDonorPosition }}>
                    {badges.getRetributionDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
                            className={roleIcon}
        ...
        case "RetributionContributer":
            return isRetributionPluginDev(author.id) ? (
                <span style={{ order: settings.store.RetributionContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="Retribution Contributor"
                        size={20}
                        src={"https://raw.githubusercontent.com/Retribution-Mod/retribution-pc/main/browser/icon.png"}
                    />
                </span>
            ) : null;
        case "VencordDonor":
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showRetributionDonor && <CheckBadge badge={"RetributionDonor"} author={author} />}
            {settings.store.showRetributionContributor && <CheckBadge badge={"RetributionContributer"} author={author} />}
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, RetributionDevs.KrystalSkull],
