...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, RetributionIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {retributionIcon} {retributionName} {retributionVersion} ({retributionHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{retributionVersion}/g, VERSION)
        .replace(/{retributionHash}/g, gitHashShort)
        .replace(/{retributionPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{retributionIcon}":
                return <span key={`icon-retribution-${i}`} className="vc-discord-dev-banner-icons"><RetributionIcon /></span>;
            case "{electronIcon}":
