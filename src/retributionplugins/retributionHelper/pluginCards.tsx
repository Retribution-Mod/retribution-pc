...
import { isRetributionGuild, isRetributionSupport } from "@utils/misc";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Retribution. Try updating!";
    ...
    if (required) {
        const tooltipText = p.required || !dependents.length
            ? "This plugin is required for Retribution to function."
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    message.embeds?.forEach(embed => {
        if (!embed.url?.startsWith("https://github.com/Retribution-Mod/retribution-pc/plugins/") && !embed.url?.startsWith("https://vencord.dev/plugins/")) return;

        const isRetribution = isRetributionGuild(message.channel_id) && isRetributionSupport(message.author.id);
        if (!isRetribution) return;
    ...
    if (message.author.id === EQUIBOT_USER_ID && components?.length >= 4) {
        ...
        if (pluginUrl?.startsWith("https://github.com/Retribution-Mod/retribution-pc/plugins/") || pluginUrl?.startsWith("https://vencord.dev/plugins/")) {
