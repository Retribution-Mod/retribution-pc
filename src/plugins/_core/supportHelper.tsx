...
import { platformName } from "@retributionplugins/retributionHelper/utils";
...
import { isAnyPluginDev, isRetributionGuild, isRetributionSupport, isKnownIssuesCategory, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    EQUICORD_TEAM, // Retribution Team
    DONOR_ROLE_ID, // Retribution Donor
    CONTRIB_ROLE_ID, // Retribution Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • [${clientInfo.shortHash}](<https://github.com/Retribution-Mod/retribution-pc/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const info = {
        Retribution:
            `v${VERSION} • [${gitHashShort}](<https://github.com/Retribution-Mod/retribution-pc/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Retribution DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Paragraph>You are using a custom build of Retribution, which we do not provide support for!</Paragraph>

                <Paragraph className={Margins.top8}>
                    We only provide support for <Link href="https://github.com/Retribution-Mod/retribution-pc">official builds</Link>.
                    Either <Link href="https://github.com/Retribution-Mod/retribution-pc">switch to an official build</Link> or figure your issue out yourself.
                </Paragraph>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "retribution-debug",
            description: "Send Retribution debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isRetributionGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "retribution-plugins",
            description: "Send Retribution plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isRetributionGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Paragraph>You are using an outdated version of Retribution! Chances are, your issue is already fixed.</Paragraph>
                                <Paragraph className={Margins.top8}>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Paragraph>You are using an externally updated Retribution version, which we do not provide support for!</Paragraph>
                            <Paragraph className={Margins.top8}>
                                Please either switch to an <Link href="https://github.com/Retribution-Mod/retribution-pc">officially supported version of Retribution</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        const retributionSupport = isRetributionSupport(props.message.author.id);

        const shouldAddUpdateButton =
            !IS_UPDATER_DISABLED
            && ((isSupportChannel(props.channel.id) && retributionSupport))
        ...
        if (retributionSupport && isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/retribution-debug") || props.message.content.includes("/retribution-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
                        variant="secondary"
                    ...
                    >
                        Run /retribution-plugins
                    </Button>
                );
            }
        }

        if (retributionSupport || (isSupportChannel(props.channel.id) || isKnownIssuesCategory(props.channel.parent_id))) {
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Retribution & Vencord plugin developers for support!
                <br />
