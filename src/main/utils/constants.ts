...
export const DATA_DIR = process.env.EQUICORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "RetributionData", suffix)
        : join(app.getPath("userData"), "..", "Retribution", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Retribution] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
