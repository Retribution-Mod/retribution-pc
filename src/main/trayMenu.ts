...
function createRetributionMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Retribution",
            submenu: [
                {
                    label: "About Retribution",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Retribution" : "Check for Updates",
                    click: () => sendToRenderer(IpcEvents.TRAY_CHECK_UPDATES)
                },
                {
                    label: "Repair Retribution",
