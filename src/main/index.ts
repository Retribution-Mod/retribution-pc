...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("retribution", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("retribution://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Retribution] Installed React Developer Tools"))
                    .catch(err => console.error("[Retribution] Failed to install React Developer Tools", err));
        } catch { }
