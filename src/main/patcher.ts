...
console.log("[Retribution] Starting up...");
...
if (!IS_VANILLA) {
    ...
    if (!IS_VESKTOP && !IS_EQUIBOP) {
        try {
            ...
            console.error("[Retribution] Failed to install host update hook", err);
        }
    }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Retribution");
    ...
    console.log("[Retribution] Running in vanilla mode. Not loading Retribution");
}

console.log("[Retribution] Loading original Discord app.asar");
